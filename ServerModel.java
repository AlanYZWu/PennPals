...
public final class ServerModel {
    private final TreeMap<Integer, String> users;
    private final TreeSet<Channel> channels;
    ...
    public Collection<String> getUsersInChannel(String channelName) {
        TreeSet<String> channelUsers = new TreeSet<>();
        for (Channel entry : channels) {
            if (entry.getName().equals(channelName)) {
                channelUsers.addAll(entry.getUsers());
            }
        }
        return channelUsers;
    }
    ...
    public Broadcast deregisterUser(int userId) {
        ...
        TreeSet<Channel> toRemove = new TreeSet<>();

        for (Channel entry : channels) {
            if (entry.contains(nickname)) {
                entry.removeUser(nickname);
                recipients.addAll(entry.getUsers());

                if (entry.getOwner().equals(nickname)) {
                    toRemove.add(entry);
                }
            }
        }

        for (Channel entry : toRemove) {
    ...
    public Broadcast changeNickname(NicknameCommand nickCommand) {
        ...
        } else if (this.getRegisteredUsers().contains(nickCommand.getNewNickname())) {
        ...
        for (Channel entry : channels) {
            if (entry.contains(nickCommand.getSender())) {
                if (entry.getOwner().equals(nickCommand.getSender())) {
                    entry.setOwner(nickCommand.getNewNickname());
                }
                entry.removeUser(nickCommand.getSender());
                entry.addUser((nickCommand.getNewNickname()));
                recipients.addAll(entry.getUsers());
            }
        }
    ...
    public Broadcast createChannel(CreateCommand createCommand) {
        ...
        } else if (this.getChannels().contains(createCommand.getChannel())) {
            return Broadcast.error(createCommand, ServerResponse.CHANNEL_ALREADY_EXISTS);
        }

        TreeSet<String> recipients = new TreeSet<>();
        recipients.add(createCommand.getSender());
        channels.add(
                new Channel(
                        createCommand.getSender(), createCommand.getChannel(),
    ...
    public Broadcast joinChannel(JoinCommand joinCommand) {
        if (!this.getChannels().contains(joinCommand.getChannel())) {
        ...
        for (Channel entry : channels) {
            if (entry.getName().equals(joinCommand.getChannel())) {
                if (entry.getPrivacy()) {
                    return Broadcast.error(joinCommand, ServerResponse.JOIN_PRIVATE_CHANNEL);
                }

                entry.addUser(joinCommand.getSender());
                recipients.addAll(entry.getUsers());
                owner = entry.getOwner();
            }
        }

        return Broadcast.names(joinCommand, recipients, owner);
    }
    ...
    public Broadcast sendMessage(MessageCommand messageCommand) {
        if (!this.getChannels().contains(messageCommand.getChannel())) {
        ...
        for (Channel entry : channels) {
            if (entry.getName().equals(messageCommand.getChannel())) {
                if (!entry.contains(messageCommand.getSender())) {
    ...
    public Broadcast leaveChannel(LeaveCommand leaveCommand) {
        if (!this.getChannels().contains(leaveCommand.getChannel())) {
        ...
        for (Channel entry : channels) {
            if (entry.getName().equals(leaveCommand.getChannel())) {
                if (!entry.getUsers().contains(leaveCommand.getSender())) {
                    return Broadcast.error(leaveCommand, ServerResponse.USER_NOT_IN_CHANNEL);
                }

                recipients.addAll(entry.getUsers());
                entry.removeUser(leaveCommand.getSender());
    ...
    public Broadcast inviteUser(InviteCommand inviteCommand) {
        if (!this.users.containsValue(inviteCommand.getUserToInvite())) {
            return Broadcast.error(inviteCommand, ServerResponse.NO_SUCH_USER);
        } else if (!this.getChannels().contains(inviteCommand.getChannel())) {
        ...
        for (Channel entry : channels) {
            if (entry.getName().equals(inviteCommand.getChannel())) {
                if (!entry.getPrivacy()) {
                    return Broadcast.error(inviteCommand, ServerResponse.INVITE_TO_PUBLIC_CHANNEL);
                } else if (!entry.getOwner().equals(inviteCommand.getSender())) {
                    return Broadcast.error(inviteCommand, ServerResponse.USER_NOT_OWNER);
                }

                entry.addUser(inviteCommand.getUserToInvite());
                recipients.addAll(entry.getUsers());
                owner = entry.getOwner();
            }
        }

        return Broadcast.names(inviteCommand, recipients, owner);
    }
    ...
    public Broadcast kickUser(KickCommand kickCommand) {
        if (!this.users.containsValue(kickCommand.getUserToKick())) {
            return Broadcast.error(kickCommand, ServerResponse.NO_SUCH_USER);
        } else if (!this.getChannels().contains(kickCommand.getChannel())) {
        ...
        for (Channel entry : channels) {
            if (entry.getName().equals(kickCommand.getChannel())) {
                if (!entry.contains(kickCommand.getUserToKick())) {
                    return Broadcast.error(kickCommand, ServerResponse.USER_NOT_IN_CHANNEL);
                } else if (!entry.getOwner().equals(kickCommand.getSender())) {
                    return Broadcast.error(kickCommand, ServerResponse.USER_NOT_OWNER);
                }

                recipients.addAll(entry.getUsers());
                entry.removeUser(kickCommand.getUserToKick());
            }
        }
